...
void Framebuffer::init(GLsizei width, GLsizei height, const std::vector<std::pair<std::string, RenderBuffer>>& renderBuffers, uint32_t msaaSamples)
{
...

	{
		...
		for (auto& type : this->renderBuffersV)
		{
			...
			if (type.second.attachment != GL_DEPTH_ATTACHMENT && type.second.attachment != GL_DEPTH_STENCIL_ATTACHMENT)
			{
...
void Framebuffer::blitFX()
{
	for (auto& type : this->renderBuffersV)
	{
		switch (type.second.attachment)
		{
		case GL_DEPTH_STENCIL_ATTACHMENT:
			glBlitNamedFramebuffer(FBO, FBOFX, 0, 0, w, h, 0, 0, w, h, GL_STENCIL_BUFFER_BIT, GL_NEAREST);
